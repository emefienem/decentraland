...
// // import { useMultipleInView } from "./useMultipleInView";
...
//   // Array to store the visibility status of each ref
//   const [inViewStatus, setInViewStatus] = useState(sections.map(() => false));

//   useEffect(() => {
//     const observer = new IntersectionObserver(
//       (entries) => {
//         entries.forEach((entry, index) => {
//           const newInViewStatus = [...inViewStatus];
//           newInViewStatus[index] = entry.isIntersecting;
//           setInViewStatus(newInViewStatus);
//         });
//       },
//       { threshold: 0.5 }
//     );
...
//   useEffect(() => {
//     const inViewIndex = inViewStatus.findIndex((status) => status === true);
//     if (inViewIndex !== -1) {
//       setCurrentImage(sections[inViewIndex].src);
//     }
//   }, [inViewStatus, sections]);
//   return (
//     <>
...
const Section = ({ section, setCurrentImage }) => {
  ...
  useEffect(() => {
    if (inView) {
      setCurrentImage(section.src);
    }
  }, [inView, section.src, setCurrentImage]);

  return (
    <div ref={ref} className="md:py-10 py-0 md:pl-24 pl-0">
      <div className="flex space-x-1 md:space-x-12 mb-2 md:mb-36">
        <span className="text-white text-sm md:text-[40px]">{section.id}</span>
        <h2 className="text-white text-5xl md:text-8xl font-bold">
          {section.title}
        </h2>
      </div>
